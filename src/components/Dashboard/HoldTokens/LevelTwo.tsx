...
import { holdTokensLevelTwo } from "../utils/data";
...
const LevelTwo = () => {
  return (
    <div className="flex flex-col gap-5">
      ...
      <hr className="border border-[#7D7D7D]" />
      <div className="tokens-two flex flex-row overflow-auto gap-24 mt-14 scroll-smooth">
        {holdTokensLevelTwo.map((token) => (
          <Token token={token} key={token.lineOne} />
        ))}
      </div>
      <div className="flex flex-row gap-9 items-center justify-between max-w-fit mx-auto">
        <button onClick={() => moving(Direction.left, "tokens-two")}>
          <Image
        ...
        <button onClick={() => moving(Direction.right, "tokens-two")}>
          <Image
